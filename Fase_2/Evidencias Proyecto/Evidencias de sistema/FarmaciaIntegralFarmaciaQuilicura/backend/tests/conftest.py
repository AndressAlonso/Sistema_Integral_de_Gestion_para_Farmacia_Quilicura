...
Requiere SIGFQ_TEST_ROLLBACK=1 y DB_* apuntando a una instancia local
de desarrollo con 0001_acceso aplicada.

No crea bases, tablas ni modifica migraciones.
"""
...
def setup(password_hash):
    if os.environ.get("SIGFQ_TEST_ROLLBACK") != "1":
        pytest.skip(
            "Habilita SIGFQ_TEST_ROLLBACK=1 para probar "
            "PostgreSQL local con rollback"
        )
