...
import type { InternalUser, NewUserInput, Role, UserInput } from './users.api'
...
export default function UserDialog({ editor, roles, busy, error, onClose, onSave, onDeactivate }: Props) {
  const dialog = useRef<HTMLDialogElement>(null)
  const [name, setName] = useState(editor.user?.name ?? '')
  const [email, setEmail] = useState(editor.user?.email ?? '')
  const [password, setPassword] = useState('')
  const [selectedRoles, setSelectedRoles] = useState<string[]>(editor.user?.roles ?? [])
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setValidation('')
    const data: UserInput = { name: name.trim(), email: email.trim(), roles: selectedRoles }
    await onSave(creating ? { ...data, password, is_active: isActive } : data)
  }
      ...
      <form onSubmit={submit} aria-busy={busy}>
        ...
        {deactivating ? <p>¿Desactivar a <strong>{editor.user?.name || editor.user?.email}</strong>? Su cuenta se conservará, pero ya no podrá iniciar sesión ni utilizar sus sesiones existentes.</p> : (
          <fieldset disabled={busy} className="users-fields">
            <label>Nombre completo<input autoComplete="name" value={name} onChange={event => setName(event.target.value)} required maxLength={120} /></label>
            ...
            <fieldset className="users-role-fields"><legend>Roles *</legend>{roles.map(role => <label key={role.code}><input type="checkbox" checked={selectedRoles.includes(role.code)} onChange={event => setSelectedRoles(current => event.target.checked ? [...current, role.code] : current.filter(code => code !== role.code))} />{role.name}</label>)}</fieldset>
